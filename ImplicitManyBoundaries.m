    ...
    while dist > epsilon && (k<maxrun)
        ...
        for i = 2:1:I+1
           ...
           if i < I+1
               ...
               MM = 0.5*vol^2*theta(i,n)^2 + h*(r*(i-1)*h+theta(i,n)*(b-r));
           end
           if P(i,1) < 0
        ...
        M(I+1,I+1) = (1-exp(beta*h^2/Q3(I*h)))*Q3(I*h) - M(I+1,I) -MM+MM*Gp -h^2/dt;
        
        if 1-(M(I,I-1)+MM-MM*Gp+h^2/dt)/Q3(I*h) < 0
            disp('Less Than Zero Funny Business')
        end
        

        for i = 2:1:I+1
           %d(i,1) = -exp(beta*h^2/Q((i-1)*h))*h^2*con(i,1)^g;
        ...
        for i = 2:1:I
         ...
         control1 = -(b-r)*h*(Gp*V(I+1,n)-V(I+1,n))/...
               (vol^2*(Gp*V(I+1,n)-2*V(I+1,n)+V(I,n)));
        theta(I+1,n) = min(max(control1,0),K*I*h);
        %theta(I+1,n) = 0;
...
%         con(I+1,n) = 2*con(I,n)-con(I-1,n);
    ...
    elseif q == 3
        ...
        figure(1)
        plot(wealth,thetaerror(2:end,1),':k')
        h1=legend({'I = 100,   N = 10  ','I = 400,   N = 20  ','I = 1600, N = 40  ','I = 6400, N = 80  '},'location','northwest');
    ...
    elseif q == 3
        ...
        figure(2)
        plot(wealth,conerror(2:end,1),':k')
        h2=legend({'I = 100,   N = 10  ','I = 400,   N = 20  ','I = 1600, N = 40  ','I = 6400, N = 80  '},'location','northwest');
        set(gca, 'YGrid', 'on', 'XGrid', 'off');
        set(h2,'FontSize',14)
    end
